...
var RxObservable = require("@rx-reason/rx-core/lib/js/src/RxObservable.bs.js");
var RxObservables = require("@rx-reason/rx-observables/lib/js/src/RxObservables.bs.js");
...
suite
  ...
  .add('rx-reason', function(deferred) {
    var ofArrayObs = RxObservables.ofArray(undefined, a);
    var mapObs = RxObservables.map(add1, ofArrayObs);
    var keepObs = RxObservables.keep(odd, mapObs);
    var mapObs = RxObservables.map(add1, keepObs);
    var mapObs = RxObservables.map(add1, keepObs);
    var keepObs = RxObservables.keep(even, mapObs);
    var scanObs = RxObservables.scan(sum, 0, keepObs);

    runners.runRxReason(deferred, scanObs);
  }, options)
